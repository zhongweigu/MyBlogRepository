...
* 当函数返回的对象**在函数调用结束后仍然有效**，并且希望**避免对象拷贝、支持链式调用或直接修改原对象**时，可以将函数的返回值类型定义为引用类型。
* 若函数返回的是**局部对象或临时对象的引用**，在函数返回后该对象已被销毁，引用将成为悬空引用，继续使用会导致未定义行为。
...
### （C++17）什么是any、optional、tuple？有什么用？

- `std::any` 是一个**类型擦除**（type erasure）容器，可以在**运行期**保存“任意类型”的值，但仍保持**类型安全**。
- `std::optional` 是对“**值缺失**（absence of value）”的显式建模。`std::optional<int> f(bool ok)`的含义是“可以不返回值，如果返回的话，值是int”
- `std::tuple` 是一个编译期已知长度、元素类型可不同的聚合类型。类似于长度更长的`pair`

### 什么是Visitor模式？好处是什么？

- **访问者模式（Visitor Pattern）** 是一种**行为型设计模式**，它的作用是：**在不修改已有类结构的情况下，为一组对象增加新的操作。**
- 它通过把“要对对象执行的操作”抽离到独立的访问者类中，让对象本身只负责提供访问接口。
- **数据结构（对象）保持不变**，操作（行为）集中到 Visitor 中，对象通过 `accept()` 方法接收访问者，访问者根据对象的具体类型执行不同操作
